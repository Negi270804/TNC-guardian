...
export const Documents: React.FC = () => {
  ...
  // Upload Mutation with progress hook listener
  const uploadMutation = useMutation<Document, Error, File>({
    mutationFn: async (file: File) => {
      const formData = new FormData();
      formData.append('file', file);
      
      setUploadProgress(1); // Set initial loader progress
  ...
  const validateAndUpload = (file: File) => {
    ...
    if (!ALLOWED_MIME_TYPES.includes(file.type)) {
      setErrorToast(`Unsupported file MIME Type: ${file.type || 'unknown'}. Please upload valid documents.`);
      setTimeout(() => setErrorToast(null), 5000);
      return;
    }
  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Upload and store legal terms and conditions files or privacy policies.</p>
      </div>
                  ...
                  <tbody className="divide-y divide-slate-850">
                    {documents.map((doc) => (
                      <tr key={doc.id} className="hover:bg-slate-900/40 text-slate-300 transition">
                        <td className="p-3 font-medium text-slate-100 max-w-[200px] truncate">
                        ...
                        <td className="p-3">
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-green-950 text-green-400 border border-green-800/40">
                            {doc.upload_status}
                          </span>
                        </td>
                        <td className="p-3 text-right space-x-2">
                          <button
                            ...
                            className="text-xs text-slate-400 hover:text-white px-2 py-1 bg-slate-900 rounded border border-slate-800 transition"
                          >
                            Details
                          </button>
                          <button
          ...
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-lg p-6 space-y-6">
            ...
            <div className="space-y-3 text-sm text-slate-300 bg-slate-950 p-4 rounded border border-slate-850">
              ...
              <div className="flex justify-between border-b border-slate-850 pb-2">
                ...
                <span>{formatBytes(selectedDoc.file_size)} ({selectedDoc.file_size} bytes)</span>
              </div>
              <div className="flex justify-between border-b border-slate-850 pb-2">
